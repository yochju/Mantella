...
int newuoa(double *, double *, long int *, long int *, double *, 
...
/* Subroutine */ int update_(long int *n, long int *npt, double *bmat, 
							 double *zmat, long int *idz, long int *ndim, double *vlag, 
							 double *beta, long int *knew, double *w)
{
...
/* Subroutine */ int trsapp_(long int *n, long int *npt, double *xopt, 
...
/* Subroutine */ int bigden_(long int *n, long int *npt, double *xopt, 
...
/* Subroutine */ int biglag_(long int *n, long int *npt, double *xopt, 
							 double *xpt, double *bmat, double *zmat, long int *idz, 
							 long int *ndim, long int *knew, double *delta, double *d__, 
...
/* Subroutine */ int newuob_(double *fun, long int *n, long int *npt, double *x, 
							 double *rhobeg, double *rhoend, long int *iprint, long int *
							 maxfun, void (*obj)(long int *, double *, double *), double *xbase, double *xopt, double *xnew, 
							 ...
							 double *d__, double *vlag, double *w)
{
	...
	/* Builtin functions */
//	double sqrt(double);
	/*long int s_wsfe(cilist *), e_wsfe(void), do_fio(long int *, char *, ftnlen);*/
...
L310:
	...
	//	printf(" \n    Function number = %5d      F = %20.16e \n\r       The corresponding X is: ",nf,f);
	...
	if (vquad >= zero) {
		if (*iprint > 0) {
			/*s_wsfe(&io___61);
			e_wsfe();*/
	//		printf( " \nf   Return from NEWUOA because a trust region step has failed to reduce Q.    \nf");
		}
		goto L530;
	}
	ratio = (f - fsave) / vquad;
...
L490:
		...
		//		printf( "\n\n\r    New Rho = %10.6e     Number of function values = %5d \n\r",rho,nf);
		...
		//		printf("    Least Value of F = %20.16e \n\r    The corresponding X is: ",fopt);

				//Attention
				for(i__ = 1; i__ <=*n;++i__) {
		//			printf("\n\r                              %10.6e ",(xbase[i__] + xopt[i__]));
				}
		//		printf("\n\r");
			}
			goto L90;
...
L530:
	...
	//		printf( "\n\r\n\r    At the return from NEWUOA     Number of function values = %5d",nf);    
				  ...
				  maxfun, void (*obj)(long int *, double *, double *))
{
	...
	/*We must declare the working space w*/

	if(*iprint > 0)
	{ 
	//	printf( "\r\n\r\n  CNEWUOA Version 1.0d (Released  July 2005) \r\n");
	//	printf( "           Powell M.J.D.         \r\n");
	//	printf( "    adapted in C by Matthieu GUILBERT               \r\n");
	}
	...
	if (*npt < *n + 2 || *npt > (*n + 2) * np / 2) {
		/*s_wsfe(&io___3);
		e_wsfe();*/
	//	printf("\n Return from NEWUOA because NPT is not in the required interval \n");
		goto L21;
	}
	...
	newuob_(fun,n, npt, &x[1], rhobeg, rhoend, iprint, maxfun, obj, &w[ixb], &w[ixo], &w[ixn], &w[ixp], &w[ifv], &w[igq], &w[ihq], &w[ipq], &w[ibmat], &
...
mxArray *xval[1], *fval[1];
char* fname;
double fbest;
long int ieval;
double ftarget;

typedef void (*myfunc)(long int *, double *, double *);

void func(long int* dim, double* x, double * f)
{ 
	...
	{
		ieval = ieval + 1;
		mexCallMATLAB(noutputs, fval, ninputs, xval, fname);  
		*f = *mxGetPr(fval[0]);
		fbest = *f;
	}
	else
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	fname = mxArrayToString(prhs[0]);	//printf("%s\n",fname);
	long int dim = (int)(mxGetScalar(prhs[1]));	//printf("%d\n",dim);	
	long int maxfevals = (int)(mxGetScalar(prhs[2]));	//printf("%d\n",maxfevals);
	ftarget = (double)(mxGetScalar(prhs[3]));	//printf("%f\n",ftarget);
	double  *x     = mxGetPr(prhs[4]);	//printf("%f\n",x[0]);
	double rhobegin = (double)(mxGetScalar(prhs[5]));	//printf("%f\n",rhobegin);
	double rhoend = (double)(mxGetScalar(prhs[6]));	//printf("%f\n",rhoend);
	long int npt = (int)(mxGetScalar(prhs[7]));	//printf("%d\n",npt);


	xval[0] = mxCreateDoubleMatrix(1, dim, mxREAL);
	fval[0] = mxCreateDoubleMatrix(1, dim, mxREAL);

	ieval = 0;
	long int iprint = 0;
	long int part_size = 100000;//npt*(npt+dim)+(5*dim*(dim+3)/2);
	mxArray *mxW  = mxCreateDoubleMatrix( 1, part_size, mxREAL );   
	double *w    = mxGetPr(mxW); 


	double f;
	newuoa(w, &f, &dim, &npt, x, &rhobegin, &rhoend, &iprint, &maxfevals, (myfunc)func);


	//output
	plhs[0] = mxCreateDoubleMatrix(1,dim,mxREAL); 
	double *xres = mxGetPr(plhs[0]);   
	for (int i = 0; i < dim; i++) 
		xres[i] = x[i]; 
	plhs[1] = mxCreateDoubleMatrix(1,1,mxREAL);
	double* bestval = mxGetPr(plhs[1]); 
	*bestval = fbest;
	plhs[2] = mxCreateDoubleMatrix(1,1,mxREAL);
	double* nevals = mxGetPr(plhs[2]); 
	*nevals = ieval;


	mxDestroyArray(mxW);
	mxDestroyArray(xval[0]);
