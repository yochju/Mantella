 ...
 for itest2 = 1:ntest2
    ...
    for iinstance = settings.instances
        ...
        if (useNEWUOA == 1)
            ...
            npt = 2*dim + 1;
            evalstotal = 0;
            while ((evalstotal < maxevals))
                xinit = -4 + 8*rand(dim,1);
                [xbest, fbest, nevals] = newuoa('MyFuncGeneric', dim, maxevals - evalstotal, fgeneric('ftarget'), xinit, rhobegin, rhoend, npt);
                evalstotal = evalstotal + nevals;
            end;
        end;
